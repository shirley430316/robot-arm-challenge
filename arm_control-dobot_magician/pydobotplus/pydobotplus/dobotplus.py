...
class Dobot:
    ...
    @staticmethod
    def get_dobot_port(config_path=None):
        """
        Get the Dobot port from the configuration file.
        
        Args:
            config_path: Optional path to the config file. If None, it will look for
                        'config/device_port.yaml' relative to the current file.
        
        Returns:
            str: The port name (e.g., 'COM5')
        
        Raises:
            DobotException: If the config file doesn't exist or doesn't contain a port.
        """
        if config_path is None:
            # Get the directory of the current file
            current_dir = os.path.dirname(os.path.abspath(__file__))
            # Go up two levels to reach the root of the project
            root_dir = os.path.dirname(os.path.dirname(current_dir))
            config_file = os.path.join(root_dir, 'config', 'device_port.yaml')
        else:
        ...
        try:
            ...
            raise DobotException(f"Failed to load port from config: {str(e)}")

    def __init__(self, port: Optional[str] = None, config_path: Optional[str] = None) -> None:
        ...
        if port is None:
            try:
                ...
                port = self.get_dobot_port(config_path)
                self.logger.debug(f"Using port from config: {port}")
            except DobotException:
                # If config fails, try to find the port automatically
                self.logger.debug("Config not found, trying to find port automatically")
